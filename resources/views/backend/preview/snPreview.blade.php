...
<div class="card border-info mb-3">
    <div class="card-header">
        ...
        <div align="right">
            ...
            @if($id2!=0)
            <a href="/backend/previewRegister/{{ $id2 }}/{{ $name }}/{{ $group1 }}/{{ $group2 }}/{{ $group3 }}/{{ $status1 }}/{{ $status2 }}/{{ $status3 }}"><button type="button" class="btn btn-primary"><i class="fa fa-chevron-left"></i> ก่อนหน้า</button></a>
            @endif

            @if($id3!=0)
            <a href="/backend/previewRegister/{{ $id3 }}/{{ $name }}/{{ $group1 }}/{{ $group2 }}/{{ $group3 }}/{{ $status1 }}/{{ $status2 }}/{{ $status3 }}"><button type="button" class="btn btn-primary">ถัดไป <i class="fa fa-chevron-right"></i></button></a>
            @endif

            {{-- name{{$name}} --}}

            {{-- <button class="btn btn-info" type="button" onClick="window.print()"><i class="fa fa-print"></i> พิมพ์</button> --}}
            @if(($name!="" or $name!=0) and $group1!=0 and $status1!=0)
                <a href="{{ url('backend/check/index')}}?txtname={{$name}}&txtgroup%5B%5D={{$group1}}&txtgroup%5B%5D={{$group2}}&txtgroup%5B%5D={{$group3}}&txtstatus%5B%5D={{$status1}}&txtstatus%5B%5D={{$status2}}&txtstatus%5B%5D={{$status3}}"><button id="back" name="back" type="button" class="btn btn-danger"><i class="fa fa-times"></i> ปิด</button></a>
            @elseif(($name!="" or $name!=0) and $group1!=0)
                <a href="{{ url('backend/check/index')}}?txtname={{$name}}&txtgroup%5B%5D={{$group1}}&txtgroup%5B%5D={{$group2}}&txtgroup%5B%5D={{$group3}}"><button id="back" name="back" type="button" class="btn btn-danger"><i class="fa fa-times"></i> ปิด</button></a>
            @elseif(($name!="" or $name!=0) and $status1!=0)
                <a href="{{ url('backend/check/index')}}?txtname={{$name}}&txtstatus%5B%5D={{$status1}}&txtstatus%5B%5D={{$status2}}&txtstatus%5B%5D={{$status3}}"><button id="back" name="back" type="button" class="btn btn-danger"><i class="fa fa-times"></i> ปิด</button></a>
            @elseif($group1!=0 and $status1!=0)
                <a href="{{ url('backend/check/index')}}?txtgroup%5B%5D={{$group1}}&txtgroup%5B%5D={{$group2}}&txtgroup%5B%5D={{$group3}}&txtstatus%5B%5D={{$status1}}&txtstatus%5B%5D={{$status2}}&txtstatus%5B%5D={{$status3}}"><button id="back" name="back" type="button" class="btn btn-danger"><i class="fa fa-times"></i> ปิด</button></a>
            @elseif($name!="" or $name!=0)
                <a href="{{ url('backend/check/index')}}?txtname={{$name}}"><button id="back" name="back" type="button" class="btn btn-danger"><i class="fa fa-times"></i> ปิด</button></a>
            @elseif($group1!=0)
                <a href="{{ url('backend/check/index')}}?txtgroup%5B%5D={{$group1}}&txtgroup%5B%5D={{$group2}}&txtgroup%5B%5D={{$group3}}"><button id="back" name="back" type="button" class="btn btn-danger"><i class="fa fa-times"></i> ปิด</button></a>
            @elseif($status1!=0)
                <a href="{{ url('backend/check/index')}}?txtstatus%5B%5D={{$status1}}&txtstatus%5B%5D={{$status2}}&txtstatus%5B%5D={{$status3}}"><button id="back" name="back" type="button" class="btn btn-danger"><i class="fa fa-times"></i> ปิด</button></a>
            @endif
            ...
            <ul>
                <li>
                    ...
                    <ul>
                        ...
                        <li>2) เกิดวันที่<div class="box-input" style="width:50px;">{{ @ Carbon\Carbon::parse($member->detail->dateOfBirth)->format('d') }}</div>
                            &nbsp;เดือน<div class="box-input" style="width:350px;">{{ Helper::monthThai( Carbon\Carbon::parse($member->detail->dateOfBirth)->format('m')) }}</div>
                        ...
                        <li>4)	สถานที่ที่สามารถติดต่อได้สะดวก <br>
                           ...
                           @elseif ($member->detail->addressType == 2)
                            ...
                            @if ($member->detail->addressType == 2)
                                <div class="box-input" style="width:500px;">{{ @ $member->detail->workPlaceName }}</div>
                                <br>
                            @endif
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img style="height:30px; width:30px" src="{{ url('backend/img/timesquare.png') }}"> ข้าพเจ้าขอรับรองว่าข้อมูลที่กรอกข้างต้น  และเอกสารที่แนบมาพร้อมใบสมัครเป็นความจริงทุกประการ  หากมีข้อมูลใดเป็นเท็จหรือไม่ตรงกับความเป็นจริง  ข้าพเจ้ายินยอมให้ถูกตัดสิทธิ์จากการเป็นผู้สมัครหรือผู้ถูกเสนอชื่อในครั้งนี้<br>
        ...
        <div class="form-group">
            ...
            <div align="right">
                ...
                <a href="/backend/previewRegister/{{ $id3 }}/{{ $name }}/{{ $group1 }}/{{ $group2 }}/{{ $group3 }}/{{ $status1 }}/{{ $status2 }}/{{ $status3 }}"><button type="button" class="btn btn-primary">ถัดไป <i class="fa fa-chevron-right"></i></button></a>
                @endif
                {{-- 'member','member2','liststatus','name','group1','group2','group3','status1','status2','status3' --}}
                <a href="{{ url('backend/check/index') }}?name={{$name}}&group1={{$group1}}&group2={{$group2}}&group3={{$group3}}&status1={{$status1}}&status2={{$status2}}&status3={{$status3}}"><button id="back" name="back" type="button" class="btn btn-danger"><i class="fa fa-times"></i> ปิด</button></a>
            </div>
        </div>
