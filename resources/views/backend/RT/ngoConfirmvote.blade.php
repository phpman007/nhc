                    ...
                    <table class="table table-striped table-bordered">
                        ...
                        @foreach($listpoint as $key=>$valpoint)
                            <tr>
                                ...
                                <td>
                                    {{-- No1={{$No1point}} i={{$i}} group={{$group}} groupid={{$group_id}} --}}
                                    @if($No1point > 1 and $i<$No1point)
                                        @php $i++; @endphp
                                        <a href="{{url('backend/RT/NGOselect/'.$group_id.'/'.$valpoint->id)}}"><div class="d-flex justify-content-center"><button type="button" class="btn btn-primary">เลือกผู้ชนะ</button></div></a>
