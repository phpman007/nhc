...
class FormOrganizationController extends Controller
{
  ...
  public function formView($step) {
    ...
    if(Auth::check() && Auth::user()->groupId != 4) {
  ...
  public function stepFour(Request $request) {
    ...
    $request->request->add(['startDate' => "{$request->stday}/{$request->stmonth}/{$request->styear}"]);

    $request->request->add(['endDate' => "{$request->etday}/{$request->etmonth}/{$request->etyear}"]);
    $request->validate([
      'no' 			   => 'required|max:7',
  ...
  public function stepSevent(Request $request)
  {
    try {
      ...
      if($yearNow < $year_older) {
        return redirect('form-organization/5')->withErrors(['yearOld' => "อายุน้อยกว่า 20 ไม่สามารถลงทะเบียนได้"]);
      }
      if(empty(Auth::user()->detail->pastWork1) || empty(Auth::user()->detail->pastOrganization1) || empty(Auth::user()->detail->time1)) {
        return redirect('form-organization/5#target1')->withErrors(['pastWork1' => "โปรดระบุข้อมูลนี้", 'pastOrganization1' => "โปรดระบุข้อมูลนี้", 'time1' => "โปรดระบุข้อมูลนี้"]);
      }

      $filename_gen = sprintf('NHC%04d', Auth::user()->id);
